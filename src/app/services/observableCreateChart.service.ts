...
export class ObservableCreateChartService {
      ...
      ((response:any) =>{
        ...
        if (response.my_register_products.length >= 1) {
          ...
          for (let i = 0; i < response.my_register_products.length; i++) {
            // this.totalMoney += response.my_register_products[i].units_purchased * response.my_register_products[i].price_product;
        ...
        console.log("Observador 1 (cliente) Completado");
  ...
  public async promiseTest(token: string){
